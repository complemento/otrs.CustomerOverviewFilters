...
######################################################################################################
    # COMPLEMENTO - GET FILTERS FOR THIS SCREEN
    my %CustomerOverviewFilters = %{$ConfigObject->Get('CustomerOverviewFilters')};
    my %Filters;
    # filter definition
#    my %Filters = (
#        MyTickets => {
...
#    );
    ...
    for my $FilterPrio (sort keys %CustomerOverviewFilters){
        $Filters{MyTickets}->{"$CustomerOverviewFilters{$FilterPrio}->{Name}"}->{"Name"}="$CustomerOverviewFilters{$FilterPrio}->{Name}";
        $Filters{MyTickets}->{"$CustomerOverviewFilters{$FilterPrio}->{Name}"}->{"Prio"}=$FilterPrio;
        $Filters{MyTickets}->{"$CustomerOverviewFilters{$FilterPrio}->{Name}"}->{"Search"}={
                    CustomerUserLoginRaw => $Self->{UserID},
                    OrderBy              => $OrderByCurrent,
                    SortBy               => $SortBy,
                    CustomerUserID       => $Self->{UserID},
        ...
        if(@{$CustomerOverviewFilters{$FilterPrio}->{States}}){
            $Filters{MyTickets}->{"$CustomerOverviewFilters{$FilterPrio}->{Name}"}->{"Search"}->{"States"}=
                \@{$CustomerOverviewFilters{$FilterPrio}->{States}};
        }
        if(@{$CustomerOverviewFilters{$FilterPrio}->{StateType}}){
            $Filters{MyTickets}->{"$CustomerOverviewFilters{$FilterPrio}->{Name}"}->{"Search"}->{"StateType"}=
                \@{$CustomerOverviewFilters{$FilterPrio}->{StateType}};
        }
    }
    ...
    if ( !$DisableCompanyTickets ) {
        my $MyTickets =  dclone($Filters{MyTickets});
        $Filters{CompanyTickets}=$MyTickets;
        for my $FilterPrio (sort keys %CustomerOverviewFilters){
            $Filters{CompanyTickets}->{"$CustomerOverviewFilters{$FilterPrio}->{Name}"}->{"Search"}->{"CustomerIDRaw"}=
                [ $UserObject->CustomerIDs( User => $Self->{UserLogin} ) ];
            delete $Filters{CompanyTickets}->{"$CustomerOverviewFilters{$FilterPrio}->{Name}"}->{"Search"}->{"CustomerUserLoginRaw"};
        }
